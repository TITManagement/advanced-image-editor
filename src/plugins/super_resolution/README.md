...
1. **SRResNetモデルファイル**: 元のGIMP-MLの `model_srresnet.pth` が必要
2. **OpenCV DNNモデル**: `EDSR_x2.pb` などの `.pb` モデルは [Saafke/EDSR_Tensorflow](https://github.com/Saafke/EDSR_Tensorflow/tree/master/models) からダウンロードし、`src/plugins/super_resolution/models/` 配下に配置するか、環境変数 `SR_SUPERRES_MODELS` でディレクトリを指定してください
3. **メモリ**: 大きな画像はパッチ処理を使用してください
4. **GPU**: CUDA対応GPUがある場合は自動で使用されます
5. **画像形式**: OpenCV読み込み可能な形式（JPEG, PNG等）
